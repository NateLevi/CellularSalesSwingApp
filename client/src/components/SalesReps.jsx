...
const SalesReps = () => {
  ...
  // Handle "Help Next Customer" button click
  const handleHelpNextCustomer = async () => {
    try {
      const token = await user.getIdToken();
      
      // Update the logged-in rep's record: only set status to "busy"
      await axios.patch(
      ...
      // update next waiting customer's record
      await axios.patch(
  ...
  const handleFinishWithCustomer = async () => {
    try {
      ...
      // Call the new finish endpoint
      await axios.patch(
        `${import.meta.env.VITE_API_URL}/api/salesreps/finish`, // <<< Added / here
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
      
      // Re-fetch updated reps data
      const newRepsResponse = await axios.get(`${import.meta.env.VITE_API_URL}/api/salesreps`);
  ...
  const handleResetRep = async () => {
    ...
    try {
      ...
      console.log("Rep reset:", response.data);
      // Re-fetch updated rep data
      const newRepsResponse = await axios.get(`${import.meta.env.VITE_API_URL}/api/salesreps`);
      setReps(newRepsResponse.data);
    } catch (err) {
  ...
  .sort((a, b) => {
    ...
    // Both are available (or both busy, in which case order doesn't change)
    if (a.status.toLowerCase() === 'available' && b.status.toLowerCase() === 'available') {
      // If both have a finished_at, sort by finished_at ascending (earlier finish comes first)
